...
the state of agent may resolve to a particular response, its state may
be subject to fast-paced change, when compared with the time it takes
to communicate across the network. This renders the response relevant
only for a short period of time, if at all, when delivered. Follow-up
requests will be relevant for an even shorter period of time.
...
attempt to keep track of the fast-paced changes at a remote agent,
while still gaining little confidence that we can act on those changes
in due time.  
